...
std::tuple<Eigen::Vector4d, Eigen::Vector3d> CPnP(const std::vector<Eigen::Vector2d>& points_2d,
  ...
  qvec_GN = Eigen::Vector4d(quat_GN.x(), quat_GN.y(), quat_GN.z(), quat_GN.w());
  tvec_GN = t_GN;

  return std::make_tuple(qvec_GN, tvec_GN);
}
